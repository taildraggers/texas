"""Aggregates Texas-located listings from every taildraggers.com
...
ones whose location ends in ", TX".
...
Pennsylvania/Rhode Island/South Carolina/South Dakota/Tennessee repos'
approach, with the state filter swapped to "TX".
"""
...
PAGE_TITLE = "Taildragger Ads in Texas"
STATE_ABBR = "TX"
...
_TEXAS_LOCATION_RE = re.compile(r",\s*TX\s*$", re.IGNORECASE)


def _is_texas(location: str) -> bool:
    return bool(_TEXAS_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        texas_rows = [row for row in rows if _is_texas(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(texas_rows)} in Texas")
        all_listings.extend(texas_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Texas listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Texas listings: {len(listings)}")
