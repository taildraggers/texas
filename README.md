...
Daily aggregator of Texas-located taildragger classified listings,
...
[Tennessee](https://github.com/taildraggers/tennessee) repos' approach,
with the state filter swapped to "TX".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, TX` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Texas"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/texas/`
...
```html
<iframe
  src="https://taildraggers.github.io/texas/"
  title="Taildragger Ads in Texas"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_TEXAS_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
